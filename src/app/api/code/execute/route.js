...
export async function POST(request) {
  ...
  try {
    ...
    if (body.problemId) {
      ...
      if (!tests.length) return Response.json({ error: "Public test cases are not configured for this problem yet." }, { status: 400 });
      const results = [];
      for (const test of tests) {
        const result = normalizeResult(await executeCode({ language: "cpp", sourceCode: body.code, stdin: test.input, timeoutSeconds: 2 }));
        results.push({ passed: result.statusId === 3 && normalizeOutput(result.stdout) === normalizeOutput(test.expectedOutput), result });
      }
