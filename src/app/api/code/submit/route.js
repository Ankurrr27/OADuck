...
export async function POST(request) {
  ...
  if (!session?.user?.email) return Response.json({ error: "You must be signed in to submit code." }, { status: 401 });

  try {
    const body = await request.json();
    const problemId = body.problemId || body.questionId;
    if (!problemId || body.language !== "cpp" || typeof body.code !== "string") return Response.json({ error: "Problem, C++, and code are required." }, { status: 400 });
    if (body.code.length > 50000) return Response.json({ error: "Code must be 50,000 characters or fewer." }, { status: 413 });

    const question = await prisma.question.findUnique({ where: { id: problemId }, select: { id: true, testCases: { select: { id: true, input: true, expectedOutput: true, isSample: true }, orderBy: { id: "asc" } } } });
    if (!question) return Response.json({ error: "Problem not found." }, { status: 404 });
    const hiddenCases = question.testCases.filter((testCase) => !testCase.isSample);
    if (!hiddenCases.length) return Response.json({ error: "Hidden test cases are not configured for this problem yet." }, { status: 400 });

    const user = await prisma.user.findUnique({ where: { email: session.user.email }, select: { id: true } });
    if (!user) return Response.json({ error: "User not found." }, { status: 401 });
    let practiceSession = await prisma.session.findFirst({ where: { userId: user.id, questionId: question.id, terminationReason: null }, orderBy: { id: "desc" } });
